...
def ajax_create_review(request):
    try:
        ...
        selected_personalities = data.get('selected_personalities', [])
        ...
        if len(selected_personalities) == 0:
            return JsonResponse({'error': '적어도 하나의 성격을 선택해주세요.'}, status=400)

        if len(selected_personalities) > 3:
            messages.error({'error': '최대 3개까지만 선택할 수 있습니다.'}, status=400)
        ...
        for personality in selected_personalities:
            if hasattr(review, personality):
                setattr(review, personality, True)
        review.save()

        return JsonResponse({
            'success': True,
            'message': '후기가 성공적으로 작성되었습니다.' if created else '후기가 성공적으로 수정되었습니다.'
        })
